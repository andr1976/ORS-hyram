...
class Fluid:
    def __init__(
        self, species=None, T=None, P=None, rho=None, phase=None, v=0, therm=None
    ):
        """
        ...
        self.rho, self.P, self.T, self.phase = (
            params["D"],
            params["P"],
            params["T"],
            params["phase"],
    ...
    def update(self, T=None, P=None, rho=None, v=None):
        ...
        if T is not None or P is not None or rho is not None:
            params = self.therm._init_fluid_params(
                {"D": rho, "P": P, "T": T, "phase": None}
            )
            self.rho = params["D"]
            self.P = params["P"]
            self.T = params["T"]
...
class Orifice:
    ...
    def flow(
        self, upstream_fluid, downstream_P=101325, mdot=None, suppressWarnings=True
    ):
        """
        ...
        """
        h0 = upstream_fluid.therm.get_property(
            "H0", P=upstream_fluid.P, D=upstream_fluid.rho, v=upstream_fluid.v
        )
        if upstream_fluid.v > 0:
            s0 = upstream_fluid.therm.get_property(
                "S", H=h0, D=np.round(upstream_fluid.rho, 12)
            )
        else:  # LH2 simulations were giving weird results when calculating entropy from enthalpy
            s0 = upstream_fluid.therm.get_property(
                "S", D=upstream_fluid.rho, T=upstream_fluid.T
            )

        fluid = copy.copy(upstream_fluid)
        if fluid.P < downstream_P:
            raise ValueError(
                "Downstream pressure is higher than upstream pressure.  Nonphysical."
            )
        if fluid.P == downstream_P:
            if mdot is not None:
                ...
                raise ValueError(
                    "Downstream pressure is the same as upstream pressure.  Need to specify mass flow rate (mdot)."
                )
        ...
        P = optimize.minimize_scalar(
            negflux, bounds=(downstream_P, upstream_fluid.P), method="bounded"
        )["x"]
        h, rho = fluid.therm.get_property(["H", "D"], P=P, S=s0)
        fluid.update(rho=rho, P=P, v=np.sqrt(2 * (h0 - h)))
        if P - downstream_P > 0.01:
            if mdot is not None:
                if not suppressWarnings:
                    warnings.warn(
                        "Fluid choked. Ignoring mdot specification and using choked flow calculation.",
            ...
            if mdot is None:
                if not suppressWarnings:
                    warnings.warn(
                        "Fluid unchoked. Provide a mass flow rate.",
                        category=PhysicsWarning,
                    )
            else:
                ...
                fluid.update(
                    rho=rho,
                    P=(
                        P * (P - downstream_P > 0.01)
                        + downstream_P * (P - downstream_P <= 0.01)
                    ),
                    v=v,
                )
        return fluid
...
class Source(object):
    ...
    def empty(
        ...
        r.set_initial_value([m0, u0]).set_f_params(
            volume, orifice, heat_flux, ambient_P
        )
        ...
        if t_empty is None:
            ...
            dt = t_empty
        times, fluid_list, mdot, sol, pres = [], [], [], [], []
        ...
        while True:
            ...
            try:
                ...
                for i in range(100):
                    try:
                        ...
                        print(
                            i,
                            "unable to advance past time %.5f s with remaining mass of %.3f g, P = %.1f Pa"
            ...
            if t_empty is not None:
                ...
                r.y[0] < m_empty
                or fluid_list[-1].P < (1 + p_empty_percent / 100) * ambient_P
            ):
                break
        self.mdot, self.fluid_list, self.ts, self.sol, self.pres = (
            mdot,
            fluid_list,
            times,
            np.array(sol).T,
    ...
    def plot_time_to_empty(self):
        ...
        [a.grid(which="major", color="k", dashes=(2, 2), alpha=0.5) for a in axs]
        [a.grid(which="minor", color="k", alpha=0.1) for a in axs]
        return fig
...
class Enclosure:
    ...
    def __init__(self, H, A, H_release, ceiling_vent, floor_vent, Xwall=np.inf):
        ...
        """
        self.H, self.A, self.ceiling_vent, self.floor_vent = (
            H,
            A,
            ceiling_vent,
            floor_vent,
