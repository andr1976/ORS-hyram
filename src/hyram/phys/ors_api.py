...
def run_confined_overpressure(sheet_name, filename=None):
    ...
    if species == "mixture":
        species = {}  
        for component, molefrac in zip(sheet.range("HYRAM_COMPS").value, sheet.range("HYRAM_MOLEFRACS").value):
    ...
    if nozzle_model == "YuceilOtugen": nozzle_model = "yuce"
    if nozzle_model == "EwanMoodie": nozzle_model = "ewan"
    if nozzle_model == "Birch": nozzle_model = "birc"
    if nozzle_model == "Birch2": nozzle_model = "bir2"
    if nozzle_model == "Molkov": nozzle_model = "molk"
    ...
    release_fluid = phys_api.create_fluid(species,
                                            temp=sheet.range("STAG_TEMP").value + 273.15,  # K
                                            pres=sheet.range("STAG_PRES").value * 1e5,  # Pa
                                            phase='none')

    ambient_fluid = phys_api.create_fluid('AIR',temp=sheet.range("AMB_TEMP").value + 273.15,  # K
                                            pres=sheet.range("AMB_PRES").value*1e5)  # Pa
    ...
    result = phys_api.analyze_accumulation(ambient_fluid, release_fluid,
                         tank_volume, leak_diam, rel_height,
                         enclos_height, floor_ceil_area,
                         ceil_vent_xarea, ceil_vent_height,
                         floor_vent_xarea, floor_vent_height,
                         times, orif_dis_coeff=Cd, ceil_vent_coeff=1, floor_vent_coeff=1,
                         vol_flow_rate=0, dist_rel_to_wall=dist_rel_to_wall,
                         tmax=max_time, rel_area=None, rel_angle=rel_angle, nozzle_key=nozzle_model,
                         x0=0, y0=0, nmax=1000,
                         temp_pres_points=None, pres_ticks=None, is_steady=is_steady,
                         create_plots=True, output_dir='out', verbose=False)    
    
    
    sheet.pictures.add(result['pres_plot_filepath'], name='Overpressure', update=True,
                        left=sheet.range('C66').left, top=sheet.range('C66').top, scale=0.9)   
    sheet.pictures.add(result['layer_plot_filepath'], name='Layer', update=True,
                        left=sheet.range('C93').left, top=sheet.range('C93').top, scale=0.9)  
    
    sheet.pictures.add(result['trajectory_plot_filepath'], name='Trajectory', update=True,
                        left=sheet.range('C121').left, top=sheet.range('C121').top, scale=0.9)   
    sheet.pictures.add(result['mass_plot_filepath'], name='Mass', update=True,
                        left=sheet.range('C145').left, top=sheet.range('C145').top, scale=0.9)  
    ...
    if not is_steady:
        sheet.pictures.add(result['mass_flow_plot_filepath'], name='Blowdown', update=True,
                            left=sheet.range('B172').left, top=sheet.range('B172').top, scale=1.3)  
    else:
        try: 
...
def run_unconfined_overpressure(sheet_name, filename=None):
    ...
    result = phys_api.compute_overpressure(calculation_method, locations,
                         ambient_fluid, release_fluid,
                         leak_diam, mass_flow=None,
                         release_angle = rel_angle, discharge_coefficient = Cd,
                         nozzle_model = nozzle_model,
                         bst_flame_speed = bst_flame_speed, tnt_factor = 0.03,
    ...
    sheet.pictures.add(result['overp_plot_filepath'], name='Overpressure', update=True,
                        left=sheet.range('B32').left, top=sheet.range('B32').top)    
    
    if calculation_method != "Bauwens":
        sheet.pictures.add(result['impulse_plot_filepath'], name='Impulse', update=True,
                            left=sheet.range('B69').left, top=sheet.range('B69').top)   
    else: 
...
def run_jet_flame(sheet_name, filename=None):
    ...
    result = phys_api.jet_flame_analysis(ambient_fluid, release_fluid, leak_diam, mass_flow=None, dis_coeff=Cd,
                       rel_angle=rel_angle,
                       nozzle_key=nozzle_model, rel_humid=rel_hum,
                       create_temp_plot=True, temp_plot_filename=None, temp_plot_title="", temp_contours=None,
                       temp_xlims=None, temp_ylims=None,
                       analyze_flux=True, flux_plot_filename=None, flux_coordinates= [(coord_x, coord_y, coord_z)], flux_contours=None,
                       flux_xlims=None, flux_ylims=None, flux_zlims=None,
                       output_dir='out', verbose=False, wind_speed=wind_speed)

    sheet.pictures.add(result[1], name='Flame heat flux', update=True,
                        left=sheet.range('B32').left, top=sheet.range('B32').top)


    sheet.pictures.add(result[0], name='Flame temperature', update=True,
                        left=sheet.range('B68').left, top=sheet.range('B68').top)
...
def run_jet_plume(sheet_name, filename=None):
    ...
    result = phys_api.analyze_jet_plume(ambient_fluid, release_fluid, leak_diam, mass_flow=None,
                        rel_angle=rel_angle, dis_coeff=Cd, nozzle_model=nozzle_model,
    ...
    sheet.pictures.add(result['plot'], name='Jet Plume', update=True,
                        left=sheet.range('B28').left, top=sheet.range('B28').top)
